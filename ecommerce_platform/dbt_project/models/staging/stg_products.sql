with source_products as (
    select * from sqlite_scan('{{ env_var("DB_PATH", "../data/ecommerce.db") }}', 'products')
),

source_inventory as (
    -- Mock inventory using sqlite products ids
    select product_id as id, 100 as stock from sqlite_scan('{{ env_var("DB_PATH", "../data/ecommerce.db") }}', 'products')
),
